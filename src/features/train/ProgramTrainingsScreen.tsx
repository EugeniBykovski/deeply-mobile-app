                ...
                {items.filter((i) => i.lastRunStatus === 'completed').length > 0 && (
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                    ...
                    <AppText variant="caption" style={{ color: '#3BBFAD' }}>
                      {t('train_done_count', { count: items.filter((i) => i.lastRunStatus === 'completed').length })}
                    </AppText>
                  </View>
                )}
                {items.filter((i) => i.lastRunStatus === 'in_progress').length > 0 && (
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                    ...
                    <AppText variant="caption" style={{ color: '#D4915A' }}>
                      {t('train_in_progress_count', { count: items.filter((i) => i.lastRunStatus === 'in_progress').length })}
                    </AppText>
                  </View>
                )}
                <AppText variant="caption" secondary>
                  {t('train_free_locked_summary', {
                    free: items.filter((i) => !i.isLocked).length,
